...
class F5BigIQPurchasedPoolLicensor(resource.Resource):
    ...
    def _delete_member(self):
        ...
        if self.license_uuid:
            ...
            self.member.member_id = self.license_uuid
            response = biq.delete(member_url, json=self.member.delete_definition())
            attempts = 30
            while True:
                if attempts == 0:
                    raise Exception(
            ...
            try:
                ...
                while member_licensing:
                    ...
                    try:
                        if not self.license_uuid:
                            members_url = \
                                '%s/purchased-pool/licenses/%s/members' % (
                                    biq.base_url,
                                    self.pool_uuid
                                )
                            response = biq.post(members_url, json=self.member.create_defintiion())
                            response.raise_for_status()
                            response_json = response.json()
                            self.license_uuid = response_json['uuid']
                            self.resource_id_set(response_json['uuid'])
                            self.member.member_id = self.license_uuid
                        member_url = '%s/%s' % (members_url, self.license_uuid)
                        response = biq.get(member_url)
                        response.raise_for_status()
                        response_json = response.json()
                        if response_json['state'] == 'LICENSED':
                            member_licensing = False
                        else:
                            member_last_state = response_json['state']
                            sleep(5)
    ...
    def _get_pool_id(bigiq_session, pool_name):
        pools_url = '%s/purchased-pool/licenses?$select=id,kind,name' %  \
